...
if (
    strpos(
        $_SERVER["PHP_SELF"],
        "/include/management/populate_selectbox.php",
    ) !== false
) {
 ...
 *
 */
function populate_payment_type_id(
 ...
 *
 */
function populate_invoice_status_id(
 ...
 *
 */
function populate_invoice_type_id(
    $defaultOption = "Select Status",
    $elementName = "",
    $cssClass = "form",
    $mode = "",
 ...
 *
 */
function populate_hotspots(
    $defaultOption = "Select Hotspot",
    $elementName = "",
    $cssClass = "form",
    $mode = "",
    $defaultOptionValue = "",
...
function get_invoice_status_id()
{
    ...
    $sql = sprintf(
        "SELECT id, value FROM %s ORDER BY value ASC",
        $configValues["CONFIG_DB_TBL_DALOBILLINGINVOICESTATUS"],
    );
    $res = $dbSocket->query($sql);
...
function get_active_plans()
{
    ...
    $sql = sprintf(
        "SELECT DISTINCT(planName) AS planName, id
                      FROM %s
...
function populate_plans(
    ...
    $sql =
        "SELECT distinct(planName), id FROM " .
        $configValues["CONFIG_DB_TBL_DALOBILLINGPLANS"] .
        " WHERE planActive = 'yes' ORDER BY planName ASC;";
    $res = $dbSocket->query($sql);
...
function get_proxies()
{
    ...
    $sql = sprintf(
        "SELECT id, proxyname FROM %s ORDER BY proxyname ASC",
        $configValues["CONFIG_DB_TBL_DALOPROXYS"],
    );
    $res = $dbSocket->query($sql);
...
function get_ippools()
{
    ...
    $sql = sprintf(
        "SELECT id, pool_name, framedipaddress FROM %s ORDER BY pool_name ASC, framedipaddress ASC",
    ...
    while ($row = $res->fetchrow()) {
        [$id, $pool_name, $framedipaddress] = $row;
...
function get_huntgroups()
{
    ...
    $sql = sprintf(
        "SELECT `id`, `groupname`, `nasipaddress`, `nasportid` FROM `%s` ORDER BY `groupname`, `nasipaddress` ASC",
    ...
    while ($row = $res->fetchrow()) {
        [$id, $groupname, $nasipaddress, $nasportid] = $row;
...
function get_payment_types()
{
    global $configValues;
    $sql = sprintf(
        "SELECT DISTINCT(value) FROM %s ORDER BY value ASC",
        $configValues["CONFIG_DB_TBL_DALOPAYMENTTYPES"],
    );
...
function get_realms()
{
    global $configValues;
    $sql = sprintf(
        "SELECT DISTINCT(realmname) FROM %s ORDER BY realmname ASC",
...
function get_users($TBL_KEY = "CONFIG_DB_TBL_RADCHECK")
{
    global $configValues;
    $sql = sprintf(
        "SELECT DISTINCT(username) FROM %s ORDER BY username ASC",
        $configValues[$TBL_KEY],
    );
    return list_from_db($sql);
}
...
function get_nas_names()
{
    global $configValues;
    $sql = sprintf(
        "SELECT nasname FROM %s ORDER BY nasname ASC",
...
function get_ratenames()
{
    global $configValues;
    $sql = sprintf(
        "SELECT DISTINCT(rateName) FROM %s ORDER BY rateName ASC",
...
function get_groups_that_have_users()
{
    global $configValues;
    $sql = sprintf(
        "SELECT DISTINCT(groupname) FROM %s ORDER BY groupname ASC",
        $configValues["CONFIG_DB_TBL_RADUSERGROUP"],
    );
    return list_from_db($sql);
}
...
function get_attributes()
{
    global $configValues;
    $sql = sprintf(
        "SELECT DISTINCT(attribute) AS attribute FROM %s ORDER BY attribute ASC",
...
function get_vendors()
{
    global $configValues;
    $sql = sprintf(
        "SELECT DISTINCT(Vendor) AS Vendor FROM %s ORDER BY Vendor ASC",
        $configValues["CONFIG_DB_TBL_DALODICTIONARY"],
    );
    return list_from_db($sql);
}
...
function get_hotspots()
{
    global $configValues;
    $sql = sprintf(
        "SELECT DISTINCT(name) FROM %s ORDER BY name ASC",
        $configValues["CONFIG_DB_TBL_DALOHOTSPOTS"],
    );
    return list_from_db($sql);
}
...
function get_batch_names()
{
    global $configValues;
    $sql = sprintf(
        "SELECT DISTINCT(batch_name) FROM %s batch_name ORDER BY batch_name ASC",
...
function populate_groups(
    ...
    $sql =
        ...
        $configValues["CONFIG_DB_TBL_RADGROUPREPLY"] .
        ")" .
        "UNION (SELECT distinct(GroupName) FROM " .
        $configValues["CONFIG_DB_TBL_RADGROUPCHECK"] .
        ");";
    $res = $dbSocket->query($sql);
...
function populate_vendors(
    ...
    $sql =
        "SELECT distinct(Vendor) as Vendor FROM " .
        $configValues["CONFIG_DB_TBL_DALODICTIONARY"] .
        " ORDER BY Vendor ASC;";
    $res = $dbSocket->query($sql);
...
function populate_realms(
    ...
    $sql =
        "SELECT distinct(RealmName) as Realm FROM " .
        $configValues["CONFIG_DB_TBL_DALOREALMS"] .
        " ORDER BY Realm ASC;";
    $res = $dbSocket->query($sql);
 ...
 * alphabetical order
 */
function populate_proxys(
    $defaultOption = "Select Proxy",
    $elementName = "",
    $cssClass = "form",
    $mode = "",
) {
    ...
    $sql =
        "SELECT distinct(ProxyName) as Proxy FROM " .
        $configValues["CONFIG_DB_TBL_DALOPROXYS"] .
        " ORDER BY Proxy ASC;";
    $res = $dbSocket->query($sql);
 ...
 *
 */
function populate_password_types(
    $elementName = "",
    $cssClass = "form",
    $mode = "",
) {
    echo "<select $mode
            ...
            <option value='SSHA3-384-Password'>SSHA3-384-Password</option>
            <option value='SSHA3-512-Password'>SSHA3-512-Password</option>
            </select>";
}
